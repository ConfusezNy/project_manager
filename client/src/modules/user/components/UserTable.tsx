...
const UserTable: React.FC<Props> = ({
    ...
    const getRoleBadge = (role: string) => {
        switch (role) {
            case "ADMIN":
                return "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300 ring-1 ring-indigo-500/20";
            case "ADVISOR":
                return "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 ring-1 ring-blue-500/20";
            default:
                return "bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-300 ring-1 ring-slate-500/20";
        }
    };
                                ...
                                <td className="px-6 py-4">
                                    {!readOnly && onRoleChange ? (
                                        <select
                                            ...
                                            className={`px-3 py-1.5 rounded-lg text-xs font-bold cursor-pointer border-0 ${getRoleBadge(user.role)} focus:ring-2 focus:ring-blue-500`}
                                        >
                                        ...
                                        <span
                                            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black tracking-widest ${getRoleBadge(user.role)}`}
                                        >
                                ...
                                {!readOnly && (
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-1">
                                            <button
                                                onClick={() => onEdit(user)}
                                                className="p-2 rounded-xl hover:bg-blue-50 dark:hover:bg-blue-900/30 text-blue-600 dark:text-blue-400 transition-all"
                                            >
                                                <Pencil size={18} />
                                            </button>
                                            <button
                                                onClick={() => onDelete(user.id)}
                                                className="p-2 rounded-xl hover:bg-rose-50 dark:hover:bg-rose-900/30 text-rose-600 dark:text-rose-400 transition-all"
                                            >
                                                <Trash2 size={18} />
                                            </button>
                                        </div>
