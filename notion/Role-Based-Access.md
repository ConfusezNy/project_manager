# 👥 Role-Based Access Control

> Permissions และ User Roles Matrix — ตรงกับโค้ดจริง (Updated: 2026-02-19)

---

## 👤 User Roles

| Role           | Description             | Dashboard Route |
| -------------- | ----------------------- | --------------- |
| 👨‍🎓 **STUDENT** | นักศึกษาที่ทำโครงงาน    | `(student)/`    |
| 👨‍🏫 **ADVISOR** | อาจารย์ที่ปรึกษาโครงงาน | `(advisor)/`    |
| 👨‍💼 **ADMIN**   | ผู้ดูแลระบบ             | `(admin)/`      |

---

## 🔐 Permissions Matrix (ตามโค้ดจริง)

| Feature                          | 👨‍🎓 Student | 👨‍🏫 Advisor | 👨‍💼 Admin |
| -------------------------------- | :--------: | :--------: | :------: |
| **ดูโครงงานตนเอง**               |     ✅     |     ✅     |    ✅    |
| **สร้างทีม**                     |     ✅     |     ❌     |    ❌    |
| **เข้าร่วมทีม**                  |     ✅     |     ❌     |    ❌    |
| **สร้าง/แก้ไข Project**          |     ✅     |     ❌     |    ❌    |
| **ลบ Project (ก่อน approve)**    |     ✅     |     ❌     |    ❌    |
| **เลือกอาจารย์ที่ปรึกษา**        |     ✅     |     ❌     |    ❌    |
| **สร้าง/แก้ไข Task**             |     ✅     |     ❌     |    ❌    |
| **ส่งงาน (Submission)**          |     ✅¹    |     ❌     |    ✅¹   |
| **ให้ Comment**                  |     ✅     |     ✅     |    ❌    |
| **อนุมัติ/ปฏิเสธโครงงาน**        |     ❌     |     ✅²    |    ❌    |
| **อนุมัติ/ปฏิเสธงาน (Submission)**|    ❌     |     ✅     |    ✅    |
| **ให้คะแนน (Grades)**            |     ❌     |     ❌     |    ✅    |
| **ดูเกรดตาม section**            |     ❌     |     ❌     |    ✅    |
| **ดูเกรดตนเอง**                  |     ✅     |     ❌     |    ❌    |
| **จัดการ Section**               |     ❌     |     ❌     |    ✅    |
| **จัดการ Event**                 |     ❌     |     ❌     |    ✅    |
| **จัดการผู้ใช้**                  |     ❌     |     ❌     |    ✅    |
| **จัดการทีม (Admin)**             |     ❌     |     ❌     |    ✅    |
| **ดูสถิติภาพรวม**                |     ❌     |     ✅     |    ✅    |

> ¹ ส่งงานได้เฉพาะสมาชิกในทีม หรือ Admin
> ² อนุมัติโครงงานเฉพาะอาจารย์ที่เป็นที่ปรึกษาของโครงงานนั้น

---

## 📍 Dashboard Routes

### Student Dashboard (`(student)/`)

| Route       | Feature      |
| ----------- | ------------ |
| `/Teams`    | จัดการทีม    |
| `/tasks`    | Kanban Board |
| `/Timeline` | ดู Timeline  |
| `/Search`   | ค้นหาโครงงาน |
| `/settings` | ตั้งค่า      |

### Advisor Dashboard (`(advisor)/`)

| Route            | Feature        |
| ---------------- | -------------- |
| `/advisorteams`  | ดูทีมที่ปรึกษา |
| `/advisor-tasks` | ดู Tasks       |
| `/Timeline`      | ดู Timeline    |
| `/settings`      | ตั้งค่า        |

### Admin Dashboard (`(admin)/`)

| Route       | Feature         |
| ----------- | --------------- |
| `/sections` | จัดการหมู่เรียน |
| `/users`    | จัดการผู้ใช้    |
| `/teams`    | จัดการทีม       |
| `/settings` | ตั้งค่าระบบ     |

---

## 🛡️ Route Protection (NestJS)

### JWT Guard

```typescript
// ทุก protected endpoint ต้องมี Guard
@UseGuards(JwtAuthGuard)
@Get('my-team')
async getMyTeam(@CurrentUser() user: JwtPayload) {
  return this.teamsService.getMyTeam(user.users_id);
}
```

### Role Guard

```typescript
// Admin-only endpoints (เช่น ให้เกรด, จัดการ Event)
@UseGuards(JwtAuthGuard, RolesGuard)
@Roles('ADMIN')
@Post()
async batchSaveGrades(@Body() dto: BatchGradesDto) {
  return this.gradesService.batchSave(dto);
}
```

### Advisor Guard

```typescript
// Advisor-only endpoints (เช่น อนุมัติโครงงาน)
@UseGuards(JwtAuthGuard, RolesGuard)
@Roles('ADVISOR')
@Put(':id/status')
async updateProjectStatus(
  @Param('id') id: string,
  @Body() dto: UpdateStatusDto,
  @CurrentUser() user: JwtPayload,
) {
  return this.projectsService.updateStatus(+id, dto, user);
}
```

### Multi-Role Guard

```typescript
// ADVISOR or ADMIN (เช่น อนุมัติ/ปฏิเสธ Submission)
@UseGuards(JwtAuthGuard, RolesGuard)
@Roles('ADVISOR', 'ADMIN')
@Patch(':id/approve')
async approveSubmission(@Param('id') id: string) {
  return this.submissionsService.approve(+id);
}
```

---

## ⚠️ หมายเหตุสำคัญ (จากโค้ดจริง)

1. **Grades เป็น Admin only** — ทั้ง GET (by section), POST (batch save), PATCH, DELETE ต้อง Admin เท่านั้น
2. **Project Status** — เฉพาะ Advisor ที่เป็นที่ปรึกษาของโครงงานนั้นเท่านั้นที่อนุมัติ/ปฏิเสธได้ (ไม่ใช่ Admin)
3. **Submission Submit** — สมาชิกทีม หรือ Admin (ไม่จำกัดเฉพาะ Student)
4. **Terms POST** — ⚠️ ไม่มี auth check ในโค้ดปัจจุบัน (ต้องเพิ่ม Admin guard ตอน migrate)
5. **Users Search** — ⚠️ ไม่มี auth check ในโค้ดปัจจุบัน
6. **Profile PATCH** — ใช้ `getServerSession` (email-based) ไม่ใช่ `getAuthUser` — ต้อง migrate เป็น JWT
7. **Sections GET** — ⚠️ ไม่มี auth check ในโค้ดปัจจุบัน (public)

---

## 👨‍🎓 Student Features

- สร้างและจัดการทีม
- เข้าร่วม/ออกจากทีม
- สร้างและแก้ไขโครงงาน (ก่อนอนุมัติ)
- เลือกอาจารย์ที่ปรึกษา
- ลบอาจารย์ที่ปรึกษา (ก่อนอนุมัติ)
- จัดการ Task (สร้าง, แก้ไข, ลบ, มอบหมาย)
- ส่งงาน (Submission) — ในฐานะสมาชิกทีม
- ดู Timeline และความคืบหน้า
- ค้นหาโครงงานย้อนหลัง
- ดูเกรดตนเอง

## 👨‍🏫 Advisor Features

- ดูทีมที่เป็นที่ปรึกษา
- ให้ Comment และคำแนะนำ
- อนุมัติ/ปฏิเสธโครงงาน (เฉพาะโครงงานที่เป็นที่ปรึกษา)
- อนุมัติ/ปฏิเสธงานที่ส่ง (Submissions)
- ดูสถิติภาพรวม

## 👨‍💼 Admin Features

- จัดการ Section (CRUD + ลงทะเบียน)
- จัดการ Event (กิจกรรมส่งงาน — CRUD + สร้าง Submission อัตโนมัติ)
- **ให้คะแนน (Grades) — Batch save, แก้ไข, ลบ**
- ดูเกรดตาม Section
- อนุมัติ/ปฏิเสธงานที่ส่ง (Submissions)
- ส่งงานแทนทีม (Submissions submit)
- จัดการผู้ใช้ทั้งหมด
- จัดการทีม (เพิ่ม/ลบสมาชิก, ลบทีม)
- ดูสถิติและรายงาน

---

> **Last Updated:** 2026-02-19
